...
export default function CreateBranchScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      const response = await apiClient.post(API_ENDPOINTS.BRANCH_CREATE, {
        name: name.trim(),
      ...
      if (response.success) {
        showToast.success(response.message || 'Branch created successfully', 'Success');
        router.back();
      } else {
        showToast.error(response.message || 'Failed to create branch', 'Error');
      }
    } catch (err: any) {
      ...
      if (apiError.status === 422) {
        ...
        showToast.error(apiError.message || 'Failed to create branch', 'Error');
      }
    } finally {
      setSubmitting(false);
    }
          ...
          <View style={[
            ...
            <View style={tw`flex-row items-center mb-3`}>
              ...
              <View style={tw`flex-1`}>
                ...
                <Text size="xs" variant="secondary">Required field (for branch user)</Text>
              </View>
            </View>
            <Input
          ...
          <View style={[
            ...
            <View style={tw`flex-row items-center mb-3`}>
              ...
              <View style={tw`flex-1`}>
                ...
                <Text size="xs" variant="secondary">Required field (min 6 characters)</Text>
              </View>
            </View>
            <Input
              placeholder="Enter password for branch user"
              value={password}
