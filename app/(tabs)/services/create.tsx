...
export default function CreateServiceScreen() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      setSubmitting(true);
      const response = await apiClient.post(API_ENDPOINTS.SERVICE_CREATE, {
        name: name.trim(),
      ...
      if (response.success) {
        showToast.success(response.message || 'Service created successfully', 'Success');
        router.back();
      } else {
        showToast.error(response.message || 'Failed to create service', 'Error');
      }
    } catch (err: any) {
      ...
      if (apiError.status === 422) {
        ...
        showToast.error(apiError.message || 'Failed to create service', 'Error');
      }
    } finally {
      setSubmitting(false);
    }
